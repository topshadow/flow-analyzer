import { Fuzz, FuzzParam, str } from '@24wings/core';
...
// 检查Bash表达式
async function checkBashExpr(prefix: string, param: FuzzParam, boundaryPrefix: string, boundarySuffix: string, fuzz: Fuzz): Promise<boolean> {
  const boundaries = [
...
async function checkWindowsPowershellExpr(prefix: string, param: FuzzParam, boundaryPrefix: string, boundarySuffix: string, fuzz: Fuzz): Promise<boolean> {
  const boundaries = [
    [';', ''], // multi stmts
...
// 检查Windows CMD表达式
async function checkWindowsCmdExpr(prefix: string, param: FuzzParam, boundaryPrefix: string, boundarySuffix: string, fuzz: Fuzz): Promise<boolean> {
  const boundaries = [
    [boundaryPrefix, boundarySuffix],
  ];
  ...
  for (const [start, end] of boundaries) {
    ...
    try {
      ...
      const payload = `${prefix}${start} set /a ${randvar}=${result.num1}-${result.num2} && call echo %${randvar}% ${end}`;
      const rsp = await fuzz.sendModifiedRequest(Object.assign(param, { value: payload }));
      const rspRaw = await rsp.text();
