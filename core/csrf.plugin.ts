...
import {HttpUtils, poc } from '@24wings/core';
interface Result {
  url: string;
  req: Request;
  rsp: Response;
}
const pluginName='csrf'
...
export async function endpointOut(req: Request, res: Response) {
  ...
  console.log(await HttpUtils.dumpRequest(req.clone()))
 await corsCheck(req, res, req.url);
...
async function csrfDetect(url: string, req: Request, rsp: Response, body: string) {
  ...
  const {response:rspWithoutCookie,duration} = await poc.httpEx(noCookie);
...
export default {name:pluginName,version:'1.0.0',endpointOut:endpointOut}