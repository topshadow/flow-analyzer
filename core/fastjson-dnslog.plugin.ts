...
export async function endpoint(request: Request) {
  if (request.method.toLowerCase() != 'post') {
    console.log('fast json 检测只支持post请求');
    return 
  }

  const fuzz = await Fuzz.fromRequest(request);
  let reqStr=await HttpUtils .dumpRequest(request.clone())
  ...
  if (highRisk.filter((v) => v == true).length > 0) {
    ...
    await db.tables.vuln.insertOne({
      ...
      type: 'fastjson-rce',
      level: VulnerabilityLevel.High,
      payload: highVersionPayload.join(','),
      requestRaw: '',
      responseRow:'',
      pluginName,
