...
export async function endpoint(request: Request) {
  ...
  const fuzz = await Fuzz.fromRequest(request.clone());
...
async function nullCheck(p: FuzzParam, fuzz: Fuzz) {
  ...
  form.append(
    p.name,
    new File([new TextEncoder().encode('phppayload')], str.generateRandomString(5) + file.name + '.php' + '%00.png', {
      type: 'text/html',
    }),
  );
...
async function mimeTypeByPass(p: FuzzParam, fuzz: Fuzz) {
  ...
  form.append(
    p.name,
    new File([new TextEncoder().encode('phppayload')], str.generateRandomString(5) + file.name, { type: mimeType }),
  );
  let res = await fuzz.fuzzPostRaw(form);
  if (res.status == 200) {
